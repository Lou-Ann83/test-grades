...
```{r}
dist_philo_logic <- Philo_logic |> count(nb_grade_pl, name = "n_students")
...
```{r}
avg_by_course <- grades |> left_join(students, by="id") |> select(-c(sex, birth_date, age)) |> summarise(avg_grade = mean(grade, na.rm = TRUE), .by = c(id, group, course_id)) 
...
```{r}
ggplot(avg_by_course, aes(x = `Xenology and Extraterrestrial Life`,y = `Roman History and Civilization`)) +
  geom_point()
```
## Question 21

```{r}
cor_by_group <- avg_by_course|> summarise(correlation = cor(`Galactic Warfare and Military Tactics`,`Roman Law and Jurisprudence`),.by = group)
...
```{r}
best_group <- cor_by_group |> slice_max(abs(correlation))|>pull(group)

Best <- avg_by_course |> filter(group == best_group)

ggplot(avg_by_course, aes(x = `Galactic Warfare and Military Tactics`,y = `Roman Law and Jurisprudence`)) +
  geom_point()
...
```{r}
finals <- avg_by_course |> left_join(students, by="id") |> select(-c(birth_date, age)) |> mutate(final_grade = mean(across(-c(id, group.x, sex)), na.rm = TRUE)) |> select(id, group, sex, final_grade) |> arrange(desc(final_grade))

```
 
 I don't think my rendered html work, it stops at question 3 but I don't have the time to fix it unfortunately
